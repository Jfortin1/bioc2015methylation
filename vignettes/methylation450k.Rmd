...
* [Quality control](#qc)
* [MethylSet, RatioSet and GenomicRatioSet](#methylset)
...
As shown in \citep{rafa:2014}, biological findings in blood samples can often be confounded with cell type composition. In order to estimate the confounding levels between phenotype and cell type composition, the function \Rcode{estimateCellCounts} depending on the package `r Biocpkg("FlowSorted.Blood.450k")`estimates the cell type composition of blood samples by using a modified version of the algorithm described in \citep{houseman}. The function takes as input a *RGChannelSet* and returns a cell counts vector for each samples:
...
title: Probe design of the 450k array 
caption: 
For Type I design, both signals are measured in the same color: one probe for the methylated signal and one probe for the unmethylated signal. For Type II design, only one probe is used. The  Green intensity measures the methylated signal, and the Red intensity measures the unmethylated signal. 
